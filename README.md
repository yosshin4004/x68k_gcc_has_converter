...
HAS.X 形式変換に変換したアセンブラソースの生成例を示します。
...
>:warning:
>Linux のディストリビューターが提供してる m68k-linux-gnu-gcc などのビルド済み gcc は、X68K と ABI が異なる（関数の戻り値が d0 または a0 に格納されていることを期待する）ため、生成されたコードを既存の X68K 資産とリンクすることは不可能です。この問題を回避するには、ここで解説しているように gcc 自体をソースコードからビルドする必要があります。
